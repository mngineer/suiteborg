...
module.exports = (robot) => {
  robot.respond(/help(?:\s+(.*))?$/i, (msg) => {
    ...
    if (filter) {
      ...
      if (cmds.length === 0) {
        msg.send(`WE HAVE NOT YET ASSIMILATED A FEATURE FOR ${filter}.`.toUpperCase());
        return
      }
    }
    ...
    if (process.env.HUBOT_HELP_REPLY_IN_PRIVATE && msg.message && msg.message.user && msg.message.user.name && msg.message.user.name !== msg.message.room) {
      msg.reply('WE HAVE TRANSMITTED TO YOUR NEW NEURAL IMPLANT. YOU\'RE WELCOME.');
...
let hiddenCommandsPattern = function hiddenCommandsPattern () {
