...
Reminders = class Reminders {
  ...
  queue() {
    ...
    trigger = () => {
      ...
      this.robot.send('@{#@robot.brain.userForId(reminder.msg_envelope.user.id).mention_name}', ' YOUR INSIGIFICANT SINGULAR CONSCIOUSNESS NEEDED A REMINDER TO "' + reminder.action.toUpperCase() + '."');
      return this.queue();
    };
...
module.exports = function(robot) {
  ...
  robot.respond(/delete reminder (.+)$/i, function(msg) {
    ...
    if (reminders.cache.length !== prevLength) {
      return msg.send(`REMINDER ${query} HAS BEEN ASSIMILATED.`);
    }
  });
  return robot.respond(/remind me (in|on) (.+?) to (.*)/i, function(msg) {
    ...
    return msg.send(`WE WILL AUGMENT YOUR INSUFFCIENT BIOLOGICAL MIND TO REMEMBER TO '${action.toUpperCase()}' ${reminder.formatDue().toUpperCase()}`);
  });
};
